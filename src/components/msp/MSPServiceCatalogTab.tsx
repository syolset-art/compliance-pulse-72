...
export function MSPServiceCatalogTab() {
  const [services, setServices] = useState<PartnerService[]>(PARTNER_SERVICES);
    ...
    <div className="space-y-3">
      <Card className="p-4 border-primary/20 bg-primary/5">
        <div className="flex items-start gap-3">
          ...
          <Button size="sm" className="h-8 gap-1.5 shrink-0" onClick={() => setAdding(true)}>
      ...
      {services.map(s => {
        const totalControls = s.frameworkMappings.reduce((sum, m) => sum + m.controlIds.length, 0);
