...
import { Shield, Settings, KeyRound, Users, FileText, ChevronDown, ChevronRight, TrendingUp, BarChart3, CheckCircle2, Circle, AlertCircle } from "lucide-react";
...
export function SecurityFoundationsWidget() {
  ...
  const requirementsByPillar = PILLARS.reduce((acc, pillar) => {
  ...
  const currentScores = PILLARS.reduce((acc, p) => {
    acc[p.key] = Math.round((byDomain[p.key]?.score || 0));
    return acc;
  }, {} as Record<string, number>);

  const historyData = generateHistoryData(currentScores, isNb);
              ...
              {PILLARS.map((pillar, index) => {
                ...
                return (
                  <div
                    key={pillar.key}
                    className={cn(
                      "rounded-lg border border-border bg-card space-y-2 overflow-hidden",
                  ...
                  >
                    ...
                    >
                      ...
                      {isExpanded ? <ChevronDown className="h-3.5 w-3.5 text-muted-foreground ml-1" /> : <ChevronRight className="h-3.5 w-3.5 text-muted-foreground ml-1" />}
                    </button>
