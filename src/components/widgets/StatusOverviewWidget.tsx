...
const getProgressColor = (status: string) => {
  switch (status) {
    ...
    case "low": return "[&>div]:bg-destructive";
    default: return "";
  }
};
...
export function StatusOverviewWidget() {
  ...
  const getStatusLabel = (status: string) => {
    switch (status) {
      case "good": return t("statusOverview.good");
      case "ok": return t("statusOverview.ok");
      case "low": return t("statusOverview.low");
      default: return "";
    }
  };
  ...
  const getDomainName = (id: string) => {
    switch (id) {
      case "privacy": return t("statusOverview.privacy");
      case "infosec": return t("statusOverview.infoSec");
      default: return t("statusOverview.aiGovernance");
    }
  };
  ...
  }, [requirements, isNorwegian, i18n.language]);
      ...
      <CardHeader className="pb-4 pt-6 px-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs text-muted-foreground">{t("statusOverview.average")}</span>
      ...
      <CardContent className="space-y-4 px-6 pb-6">
          ...
          >
            ...
            >
              <div className="flex items-center gap-3 sm:gap-4 min-w-0 flex-1">
                <div className={`p-2 sm:p-2.5 rounded-xl flex-shrink-0 transition-silk ${domain.status === "good" ? "bg-success/20 text-success" : domain.status === "ok" ? "bg-warning/20 text-warning" : "bg-destructive/20 text-destructive"}`}>
                  {domain.icon}
                </div>
                ...
                <div className="bg-background/50 rounded-lg p-3 mt-2">
                  ...
                  <div className="flex items-baseline gap-2 mb-1">
                    ...
                    <span className="text-xs text-muted-foreground">
                      {t("statusOverview.requirementsFulfilled", { current: domain.slaCurrent, total: domain.slaTotal })}
                    </span>
                  </div>
                  <Progress 
                  ...
                  >
                    ...
                    {t("statusOverview.viewReadiness", { domain: domain.name })}
                  </Button>
                </div>
              </div>
            )}
        ...
        <div className="pt-3 border-t border-border flex items-center justify-between">
          <p className="text-sm text-muted-foreground">
            {t("statusOverview.totalOnTrack", { score: totalScore })}
          </p>
          <p className="text-xs text-muted-foreground">
            {t("statusOverview.basedOnRequirements", { count: domainData.reduce((acc, d) => acc + d.slaTotal, 0) })}
          </p>
        </div>
      </CardContent>
    </Card>
