          ...
          <div className="container max-w-7xl mx-auto p-4 md:p-6 space-y-6">
            ...
            <div>
              <h1 className="text-2xl font-bold text-foreground">
                {t("customerRequests.title", "Kundeforespørsler")}
              </h1>
              <p className="text-sm text-muted-foreground mt-1">
                {t("customerRequests.subtitle", "Svar på compliance-forespørsler fra dine kunder")}
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <MetricCard
                title={t("customerRequests.metrics.total", "Totale forespørsler")}
                value={requests.length}
                icon={Inbox}
              />
              <MetricCard
                title={t("customerRequests.metrics.pending", "Ventende")}
                value={pending.length}
                icon={Clock}
                className="border-l-4 border-l-amber-400"
              />
              <MetricCard
                title={t("customerRequests.metrics.inProgress", "Under arbeid")}
                value={requests.filter((r: any) => r.status === "in_progress").length}
                icon={Send}
              />
              <MetricCard
                title={t("customerRequests.metrics.overdue", "Forfalt")}
                value={overdue.length}
                icon={AlertCircle}
                className={overdue.length > 0 ? "border-l-4 border-l-destructive" : ""}
              />
            </div>
            ...
            <div className="relative max-w-md">
              ...
              <Input
                placeholder={t("customerRequests.search", "Søk i forespørsler...")}
                value={search}
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab}>
              <TabsList>
                ...
                <TabsTrigger value="archived">
                  {t("customerRequests.tabs.archived", "Arkivert")}
                </TabsTrigger>
              </TabsList>
                  ...
                  {(tabData[tab] || []).length === 0 ? (
                    <div className="text-center py-16 text-muted-foreground">
                      ...
                      <p className="text-sm font-medium">
                        {t("customerRequests.empty", "Ingen forespørsler her")}
                      </p>
                    </div>
                  ) : (
