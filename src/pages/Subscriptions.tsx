...
export default function Subscriptions() {
  ...
  // Track which modules/tiers the user wants to activate
  const [selectedModule, setSelectedModule] = useState<ModuleId | null>(null);
  const [selectedModuleTier, setSelectedModuleTier] = useState<ModuleTier>("basis");

  // Check active modules from localStorage
  ...
  const getTotalMonthlyCost = () => {
    let total = 0;
    if (systemsActive) {
      total += MODULES.systems.tiers[systemsTier].monthly;
    }
    if (vendorsActive) {
      total += MODULES.vendors.tiers[vendorsTier].monthly;
    }
    return total;
  };
    ...
    <div className="flex min-h-screen max-h-screen bg-background overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto md:pt-11">
        <div className="container max-w-5xl mx-auto p-6 space-y-8">
          {/* Header */}
          <div>
            <h1 className="text-2xl font-semibold text-foreground">Fakturering og moduler</h1>
            <p className="text-muted-foreground">Velg modulene som passer din organisasjon. Du kan velge én eller begge.</p>
          </div>

          {/* Active modules summary */}
          {(systemsActive || vendorsActive) && (
            <div className="flex items-start gap-3 p-4 rounded-lg border bg-primary/5 border-primary/20">
              <CheckCircle2 className="h-5 w-5 text-primary shrink-0 mt-0.5" />
              <div className="space-y-1">
                <p className="text-sm font-medium text-foreground">Aktive moduler</p>
                {systemsActive && (
                  <p className="text-xs text-muted-foreground">
                    Systemmodul ({systemsTier === "premium" ? "Premium" : "Basis"}) – {formatKr(MODULES.systems.tiers[systemsTier].monthly)}/mnd
                  </p>
                )}
                {vendorsActive && (
                  <p className="text-xs text-muted-foreground">
                    Leverandørmodul ({vendorsTier === "premium" ? "Premium" : "Basis"}) – {formatKr(MODULES.vendors.tiers[vendorsTier].monthly)}/mnd
                  </p>
                )}
                {(systemsActive && vendorsActive) && (
                  <p className="text-xs font-medium text-foreground mt-1">
                    Totalt: {formatKr(getTotalMonthlyCost())}/mnd
                  </p>
                )}
              </div>
            </div>
            ...
            return (
              <div key={moduleId} className="space-y-4">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-xl font-semibold text-foreground">{mod.displayName}</h2>
                  {isActive && (
                          ...
                          <div>
                            ...
                            {billingInterval === "yearly" && (
                              <p className="text-xs text-green-600 dark:text-green-400 mt-0.5">
                                Spar {formatKr(getModuleAnnualSavingsKr(moduleId, tier))} per år
                              </p>
                            )}
                          </div>
            ...
            <CardContent className="p-6 flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Ubegrenset systemer og leverandører, dedikert kontaktperson, tilpassede integrasjoner og SLA-garantier.
                </p>
              </div>
              <Button variant="outline">Kontakt salg</Button>
            </CardContent>
          </Card>
              ...
              {["GDPR", "ISO 27001"].map((name) => (
                <Card key={name} className="border-green-200 dark:border-green-900/50">
                  <CardContent className="p-4">
                    ...
                    <p className="text-xs text-muted-foreground">Inkludert gratis i alle planer</p>
                  </CardContent>
                </Card>
              ))}
          ...
          <Card>
            <CardContent className="p-6 space-y-4">
              ...
              <RadioGroup value={paymentMethod} onValueChange={setPaymentMethod} className="space-y-3">
                <label className={`flex items-start gap-3 p-4 rounded-lg border cursor-pointer transition-colors ${paymentMethod === "card" ? "border-primary bg-primary/5" : "border-border"}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">Betal med bedriftskort eller Link-lommebok.</p>
                  </div>
                  <CreditCard className="h-5 w-5 text-muted-foreground shrink-0 mt-0.5" />
                </label>

                <label className={`flex items-start gap-3 p-4 rounded-lg border cursor-pointer transition-colors ${paymentMethod === "invoice" ? "border-primary bg-primary/5" : "border-border"}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">Motta en faktura med bankdetaljer. Betaling forfaller innen 30 dager.</p>
                  </div>
                ...
                <>
                  ...
                  <div className="space-y-4 py-4">
                    ...
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-medium text-foreground">{billingInterval === "yearly" ? "Årlig" : "Månedlig"}</span>
                    </div>
                    {billingInterval === "yearly" && (
                      <div className="flex justify-between text-sm">
                        <span className="text-muted-foreground">Besparelse</span>
                        <span className="font-medium text-green-600">
                          {formatKr(getModuleAnnualSavingsKr(selectedModule, selectedModuleTier))} per år
                        </span>
                      </div>
                    )}
                    <Separator />
